function passwordMatching(form) {
	var password = form.employee_password.value;
	var confirmPassword = form.employee_confirm_password.value;
...
function currencySelectionValidation(form) {
    ...
    switch (checkedPlan) {
        case "Daily":
            document.getElementById("final_price").value = getUSDPrice(table.rows[1].cells[1].innerHTML);
            break;
        case "Monthly":
            document.getElementById("final_price").value = getUSDPrice(table.rows[2].cells[1].innerHTML);
            break;
        case "Yearly":
            document.getElementById("final_price").value = getUSDPrice(table.rows[3].cells[1].innerHTML);
            break;
    }
	return true;
}
